...
\texttt{StateGraph} with 12 real nodes and 2 conditional edges:
...
conflicts---never averaging, never invoking an LLM.
...
and synthesises a final \texttt{AuditReport}.
...
\begin{lstlisting}[language={},title={Data Flow}]
...
Using an LLM would introduce non-deterministic scoring, risk the LLM
ignoring security overrides, add latency and cost, and make the system
harder to audit.  The Chief Justice applies 5 rules in fixed order:
...
LLM Models          & Ollama              & qwen3, deepseek-v3.1, llava \\
\bottomrule
\end{tabular}
\end{table}
...
\vfill
\begin{center}
\small\textit{Swarm Auditor v0.3.0 --- Built with LangGraph + Streamlit --- 10 Academy FDE Week 2}
\end{center}
